...
return {
        ...
        if target_citekey then
          ...
          if vim.fn.filereadable(bib_path) == 1 then
            ...
            vim.ui.select({ "Sí, crear", "No, cancelar" }, {
              ...
              -- Corregido: Validamos exactamente la opción elegida ("Sí, crear")
              if choice == "Sí, crear" then
                -- Usamos la función nativa de zk para crearla formalmente
                require("zk").new({
                  group = "bibliography",
                  dir = "Bibliografía",
                  title = target_citekey, -- Usamos el citekey como título automáticamente
                  extension = "md",
                })

                vim.notify("Nota bibliográfica creada: " .. target_citekey, vim.log.levels.INFO)
              end
            end)
          end
        else
  ...
  -- Configuración de atajos de teclado para Zettelkasten usando which-key.nvim
  {
